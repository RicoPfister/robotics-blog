<template>
    <MainLayout>
        ...
        <div v-if="commentUser.length > 0">
            ...
            <div class="container p-2 rounded mx-auto display">
                    ...
                    <div v-for="commentDetail in commentUser" class="grid grid-cols-12 gap-4 p-2 rounded drop-shadow-xl m-3 bg-sky-400 text-white">
                        <div class="flex flex-col col-span-2">
                            <div class="font-bold">{{ commentDetail.name }}</div>
                            <div class="text-xs">{{ formatDate(commentDetail.created_at) }}</div>
                        </div>
         ...
         <div class="flex justify-center mb-5" v-else>No Comments found. Be the first to comment this article!</div>
    </MainLayout>
</template>
...
function formatDate(dateString) {
    ...
    const humanDateFormat = dateObject.toLocaleString('de-CH', {timeStyle: 'short', dateStyle: 'short' })
